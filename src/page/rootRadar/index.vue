...
export default {
  ...
  data() {
    return {
      showGuidance: false,
      editorExtensionId: 'bjddgpdnmmkpmdhoajcfpgkfnjkmbcmj',
      currentShopid: '',
      ...
      pickerOptions: {
        ...
        },
        {
          text: '自定义',
          onClick(picker) {
            const aaa = document.querySelectorAll('.el-picker-panel__shortcut')[1];
  ...
  methods: {
    handleScroll() {
      ...
      const originalStyle = window.getComputedStyle(ele);
      if (scrollTop > 93.6) {
        ele.style.position = 'fixed';
        ele.style.width = 'calc(100% - 497px)';
        ele.style.margin = '-100px 0 0 0';
      } else {
    ...
    turnEndDataToJson(data) {
      ...
      const nUvList = [];
      const allUvList = [];
      const zConversionList = [];
      const nConversionList = [];
      const allConversionList = [];
      const zPayCntList = [];
      const nPayCntList = [];
      const allPayCntList = [];
      const zAddCartList = [];
      const nAddCartList = [];
      const allAddCartList = [];
      const zAddFavList = [];
      const nAddFavList = [];
      const allAddFavList = [];
      const dataTimeList = [];
