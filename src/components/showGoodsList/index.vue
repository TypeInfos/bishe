...
export default {
  ...
  methods: {
    ...
    handleScroll() {
      const ele = document.querySelector('.aside');
  ...
  watch: {
    focusOn(val) {
      ...
      if (val.length > 0 && this.firstComplete) {
        this.filterFocusOn = this.focusOn;
        this.firstComplete = false;
        const haveCookie = this.$cookies.isKey('finalItemId');
        if (haveCookie) {
          const finalItemId = this.$cookies.get('finalItemId');
